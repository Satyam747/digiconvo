...
async function api(action, data = {}) {
    ...
    if (isAdditional && orderId) {
        ...
        const { data: existingOrder, error: fetchError } = await supabaseClient
            .from('orders')
            .select('cart, subtotal, discount, gst, total')
            .eq('id', orderId)
            .single();

        if (fetchError) {
            return { success: false, message: "Could not fetch existing order." };
        }

        const existingCart = existingOrder.cart || [];
        const newCart = data.cart;

        // Merge: if same item, add quantity; else push new
        newCart.forEach(newItem => {
            const found = existingCart.find(item => item.id === newItem.id);
            if (found) {
    ...
    .update({
        ...
        notes: data.order.notes || existingOrder.notes   // ← ADD THIS LINE
    })
    .eq('id', orderId);
    ...
    // (Keep your existing order insertion code here)
    // We'll use the same checks you already have.
    // Make sure you generate invNo, kotNo as before.
    // ... (your existing tableCheck, stockCheck, etc.)
...
case "getOrders": 
    ...
    // Ensure table number is a string (or number) – adjust if your DB uses numbers
