...
async function api(action, data = {}) {
    ...
    async function validateStock(cartItems) {
        for (let item of cartItems) {
            const { data: menuItem, error } = await supabaseClient
                ...
                .single();
            if (error || !menuItem) {
                return { success: false, message: `Item ${item.name} not found.` };
            }
            if ((menuItem.stock || 0) < item.qty) {
                return { success: false, message: `Insufficient stock for ${item.name}. Available: ${menuItem.stock}` };
            }
        }
        return { success: true };
    }
    ...
    if (isAdditional && orderId) {
        const { data: existingOrder, error: fetchError } = await supabaseClient
            .from('orders')
            .select('cart, subtotal, discount, gst, total, status')
            .eq('id', orderId)
            .single();
        ...
        // Deduct stock for new items only
        if (newItems.length > 0) {
