    ...
    try {
        ...
        switch(action) {
            ...
            case "placeOrder": 
                ...
                result = await supabaseClient.from('orders').insert({
                ...
                return { success: !result.error, orderId, invoiceNo: invNo, kotNo, message: result.error?.message };

            case "getOrders": 
                result = await supabaseClient.from('orders').select('*').order('created_at', { ascending: false });
                const legacyOrders = (result.data || []).map(row => ({
                    id: row.id, dateTime: new Date(row.created_at).toLocaleString('en-IN'),
            ...
            case "getDashboard": 
                const rawOrders = await supabaseClient.from('orders').select('*');
            ...
            case "getReports": 
                const reportsOrders = await supabaseClient.from('orders').select('*');
            ...
            case "getActivity": 
                ...
                const legacyActivity = (result.data || []).map(row => ({
                    time: row.time, user: row.user_role, action: row.action, details: row.details, icon: "fa-solid fa-circle-info"
                }));
                return { success: !result.error, data: legacyActivity };
            ...
            case "getKOT": 
                ...
                (kotOrder.cart || []).forEach(item => { kotRows += `<tr><td align="left">${item.name}</td><td align="right"><b>${item.qty}</b></td></tr>`; });
                let notesHtml = ""; if (kotOrder.notes && kotOrder.notes !== "") { notesHtml = `<hr><div style="border: 1px solid black; padding: 8px; margin-top: 10px;"><b>NOTES:</b><br>${kotOrder.notes}</div>`; }
                const kotHtml = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>KOT - ${kotOrder.id}</title><style>body { font-family: monospace; padding: 20px; font-size: 16px; margin: 0; } .center { text-align: center; } table { width: 100%; border-collapse: collapse; margin-top: 15px; } td { padding: 8px; border-bottom: 1px dashed #888; } hr { border: none; border-top: 1px dashed black; margin: 12px 0; } @media print { body { padding: 0; } }</style></head><body><div class="center"><h2>${kSet.restaurant_name || "Restaurant"}</h2><h3>KITCHEN ORDER TICKET</h3></div><hr><div><b>KOT :</b> ${kotOrder.kot_no}<br><b>Order :</b> ${kotOrder.id}<br><b>Table :</b> ${kotOrder.table_number}<br><b>Customer :</b> ${kotOrder.customer_name}<br><b>Time :</b> ${new Date(kotOrder.created_at).toLocaleString('en-IN')}</div><hr><table><thead><tr><th align="left">Item</th><th align="right">Qty</th></tr></thead><tbody>${kotRows}</tbody></table>${notesHtml}<hr><div class="center"><b>PREPARE IMMEDIATELY</b></div><script>window.onload = function() { window.print(); };</script></body></html>`;
