...
case "getOrders": 
    ...
    return { success: !result.error, data: legacyOrders };
            ...
            case "getOrderStatus": 
                ...
                // FIX: Automatically free the physical table if the order is Cancelled OR Completed
                if (['Cancelled', 'Completed'].includes(data.status) && !result.error) {
                ...
                // Bubbles up the exact Supabase error message to the UI toast
                return { success: !result.error, message: result.error?.message };
